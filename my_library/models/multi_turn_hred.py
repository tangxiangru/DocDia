...
class MultiTurnHred(Model):
    ...
    """
    def _encode_document(self, document: Dict[str, torch.LongTensor]) -> Dict[str, torch.Tensor]:
        # shape: (batch_size, sequence_num, sequence_length, embedding_dim)
        embedded_document = self._token_embedder(document)
        # shape: (batch_size, sequence_num, sequence_length)
        document_mask = util.get_text_field_mask(document, 1)

        batch_size, sequence_num, sequence_length = document_mask.size()

        # shape: (batch_size * sequence_num, encoder_output_dim)
        sequence_vec = self._sequence_encoder(embedded_document.view(batch_size*sequence_num, sequence_length, -1),
                                              document_mask.view(batch_size*sequence_num, -1))
        # DEBUG: assert sequence_vec.size() == (batch_size * sequence_num, self._encoder_output_dim)
        # shape: (batch_size, sequence_num, encoder_output_dim)
        sequence_vec = sequence_vec.view(batch_size, sequence_num, -1)
        # DEBUG: assert sequence_vec.size() == (batch_size, sequence_num, self._encoder_output_dim)
        # shape: (batch_size, sequence_num, encoder_output_dim)
        context_encoder_output = self._context_encoder(sequence_vec, document_mask[:, :, 0])
        # DEBUG: assert context_encoder_output.size() == (batch_size, sequence_num, self._encoder_output_dim)
        # shape: (batch_size, encoder_output_dim)
        document_vec = util.get_final_encoder_states(context_encoder_output, document_mask[:, :, 0])
        # DEBUG: assert document_vec.size() == (batch_size, self._encoder_output_dim)

        return {'document_vec': document_vec}
    """
    ...
    def take_step(self,
        ...
        ----------
        last_predictions : ``torch.Tensor``
            A tensor of shape ``(batch_size * sequence_num,)``, which gives the indices of the predictions
            during the last time step.
        state : ``Dict[str, torch.Tensor]``
            A dictionary of tensors that contain the current state information
            needed to predict the next step, which includes the encoder outputs,
            the source mask, and the decoder hidden state and context. Each of these
            tensors has shape ``(batch_size * sequence_num, *)``, where ``*`` can be any other number
            of dimensions.
